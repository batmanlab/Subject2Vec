...
parser.add_argument('-i', '--input_csv', type=str, default='../Output/Subject2Vec_Input_Updated.csv', help='CSV file containing: sid, patch, fold')
...
parser.add_argument('-cf', '--confounder', type=str, default='Age_Enroll,ATS_PackYears,gender',help='A comma separated list of subject-level confounders. e.g. --confounder "Age_Enroll,ATS_PackYears,gender"')
...
parser.add_argument('-t', '--clinical_phenotype_datatype', type=str, default='float-log,float-log',help='A comma separated list of datatype of the clinical phenotypes. e.g. --clinical_phenotype_datatype "float,float". Use "float" for regression, "float-log" for regression on log,"binary" for 2 class classification, "multi" for multi class classification and "ordered-multi" for ordered multi class classification like in BODE index. Binary, multi and ordered-multi should be of type int.')
...
parser.add_argument('-n', '--experiment_name', type=str, default='Save_Results_Exp', help='Name of the experiment')
parser.add_argument('-c', '--past_checkpoint', type=str, default='Test_Regression_Fold_1', help='Name of the past checkpoint expeiment from where to continue training')
...
parser.add_argument('-e', '--no_of_epoch', type=int, default=1, help='Number of epochs for training.')
...
if a.past_checkpoint == None:
    print("Error!! Provide a valid checkpoint to save results from")
    sys.exit()
...
def train():
    ...
    for epoch in range(a.no_of_epoch):
        ...
        while True:
            ...
            [_subjectFeature, _loss, _X_Reconstructed, _HiddenRepresentation, _y_predicted, _loss_generative, _loss_discriminative, _attentionWeights, _loss_regularization] = sess.run([subjectFeature, loss, X_Reconstructed, HiddenRepresentation, y_predicted, loss_generative, loss_discriminative, attentionWeights, loss_regularization], feed_dict={x:xs, isTrain:False, y_: ys, c_:cs, Nb:xs.shape[0]}) 
            ...
            current_output_dir = os.path.join(a.output_dir, ns)
            if not os.path.exists(current_output_dir):
                os.makedirs(current_output_dir)
            np.save(os.path.join(current_output_dir, ns + '_HiddenRepresentation_BeforeAttention.npy'), np.reshape(_HiddenRepresentation,[-1,128]))
            np.save(os.path.join(current_output_dir, ns + '_attentionWeights.npy'), _attentionWeights)
            np.save(os.path.join(current_output_dir, ns + '_subjectFeature.npy'), _subjectFeature)
            i += 1
            print(i)
        total_loss /= i
        loss_g /= i
