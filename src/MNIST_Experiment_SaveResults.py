...
parser.add_argument('--data_dir', type=str, default='/pghbio/dbmi/batmanlab/singla/MNIST_Data/', help='Directory where MNIST data is stored')
parser.add_argument('--experiment_name', type=str, default='new_MNIST_Exp_SaveResults', help='Name of the experiment')
parser.add_argument('--no_of_epoch', type=int, default=1,
...
parser.add_argument( '--num_samples', type=int, default=100, help='Number of sample sets from test set.')
...
def train():
    ...
    if a.past_checkpoint is not None:
        ...
        print("Error!! Need a checkpoint to save results from")
        sys.exit()
    ...
    for epoch in range(a.no_of_epoch):
        ...
        while sample!=a.num_samples:
            ...
            [_accuracy, _loss_generative, _loss_discriminative, _loss_regularization, _loss, ce1, ce2, _y_predicted, attention_weights] = sess.run([accuracy, loss_generative, loss_discriminative, loss_regularization, loss, crossEntropy_1, crossEntropy_2, y_predicted, alpha], feed_dict={x:xs, isTrain:False, y_: ys, n:xs.shape[0],labels:yys}) 
            r = np.random.randint(10)
            if r % 3 == 0:     
                np.save(os.path.join(log_dir, 'set_'+str(sample)+'.npy'), xs)
                np.save(os.path.join(log_dir, 'set_'+str(sample)+'_label.npy'), ys)
                np.save(os.path.join(log_dir, 'set_'+str(sample)+'_digits.npy'), yyys)
                np.save(os.path.join(log_dir, 'set_'+str(sample)+'_predicted_label.npy'), _y_predicted)
                np.save(os.path.join(log_dir, 'set_'+str(sample)+'_attention.npy'), attention_weights)
                sample +=1
            count += 1
